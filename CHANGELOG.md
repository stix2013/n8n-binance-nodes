...
### Fixed
- **API Healthcheck**: Added missing `healthcheck` to `api` service in `docker-compose.yml` to satisfy n8n `service_healthy` dependency.
- **Docker/API Configuration**: Synchronized versions across `main.py`, `Dockerfile.api`, and `docker-compose.yml`.

## [1.7.1] - 2026-03-05

### Changed
- **n8n Version**: Upgraded n8n from `2.9.4-amd64` to `2.10.3-amd64`.
  - Updated `N8N_VERSION` in `.env`, `env-example`, `docker-compose.yml`, and `dockers/Dockerfile.runners`.
- **API Version**: Upgraded project and API version to `1.7.1`.
...
### Added
- **API Response Enhancement**: Added `interval` field to price and technical analysis responses for better data traceability.
- **Troubleshooting Guide**: Added new Troubleshooting section to `AGENTS.md` to assist with common environment and runner issues.

### Fixed
- **Docker Deployment**: Fixed `.env` file mounting issues by removing read-only flag from API volume.
- **Zrok Integration**: Enabled headless mode for Zrok to prevent TTY-related errors in headless environments.

### Changed
- **Project Version**: Bumped global project version from `1.2.0` to `1.3.0`.
- **Documentation**: Updated `README.md` with accurate project structure and expanded `BinanceOrder` node documentation.

## [1.2.1] - 2026-02-09

### Changed
- **n8n Version**: Upgraded n8n from `2.6.3` to `2.6.4-amd64`
  - Updated `N8N_VERSION` in `.env` and `env-example`
  - Critical security patch for Merge Node and other core components
...
### Added
- **Startup Automation**: Added BunJS scripts for environment management
  - `bun start`: Starts Docker Compose and Zrok tunnel
  - `bun stop`: Gracefully stops Docker services and Zrok tunnel
  - Added `package.json` with convenience scripts
- **Zrok Integration**: Added Zrok configuration to environment
  - Added `ZROK_RESERVED_NAME` and `ZROK_PUBLIC_URL` to `.env`
  - Integrated Zrok tunnel management into startup script

### Changed
- **n8n Version**: Upgraded n8n from `2.5.0` to `2.6.0`
  - Updated `N8N_VERSION` default in `.env` and Dockerfile
- **Project Structure**: Added `scripts/` directory for automation tools
- **Version Bump**: Marking stable release `1.0.0`
